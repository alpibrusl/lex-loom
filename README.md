...
Agent runtime (runner, trace, state store, registry, A2A) is vendored directly into `src/agent/` — loom's agent loop itself has no external runtime dependency. Around it, three shared packages place loom in the wider Lex ecosystem: `lex-ctl` (the Operate loop's mechanism kernel, also consumed independently by [lex-soft](https://github.com/alpibrusl/lex-soft) — loom's cross-org counterpart), `lex-os-manifest` (the grant type optional [lex-os](https://github.com/alpibrusl/lex-os) sandboxing is built on), and [lex-soft](https://github.com/alpibrusl/lex-soft) itself (since #177: companies register outward-facing roles on its mesh and settle revenue through its evidence-gated settlement). For how loom, lex-ctl, lex-soft, and lex-os fit together — what's wired vs. designed vs. not started — see [lex-lang's ecosystem model](https://github.com/alpibrusl/lex-lang/blob/main/docs/design/ecosystem-model.md).
