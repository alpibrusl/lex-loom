...
if (cd "$W/echo" && bash "$OLDPWD/bin/check-derived-values.sh" . >/dev/null 2>&1); then ok "a literal that appears as input and expected value is accepted"; else bad "the round-trip assertion tzc11 denied ten times is still flagged"; fi
...
if (cd "$W/oracle" && bash "$OLDPWD/bin/check-derived-values.sh" . >/dev/null 2>&1); then bad "a literal with no input to pin it was accepted -- the check no longer checks anything"; else ok "a lone oracle is still flagged"; fi
...
if python3 -c 'import pytest' >/dev/null 2>&1; then
  if (cd "$W/coll" && bash "$OLDPWD/bin/check-derived-values.sh" . >/dev/null 2>&1); then bad "a wrong module-level pin that kills collection was accepted -- tzc13's 7 tests never ran"; else ok "an uncollectable suite is denied where it was written"; fi
else
...
if (cd "$W/lexsuite" && bash "$OLDPWD/bin/check-derived-values.sh" . >/dev/null 2>&1); then ok "a derived Lex suite passes without pytest ever running"; else bad "a Lex-only suite was denied -- the 0/5 Lex test_author baseline"; fi
...
rc=0; out=$(cd "$W/pinbad" && bash "$OLDPWD/bin/check-derived-values.sh" . 2>&1) || rc=$?
...
if (cd "$W/pinok" && bash "$OLDPWD/bin/check-derived-values.sh" . >/dev/null 2>&1); then ok "a correct pin passes"; else bad "a correct pin was denied"; fi
...
if python3 -c 'import pytest' >/dev/null 2>&1; then
  if (cd "$W/noapp" && bash "$OLDPWD/bin/check-derived-values.sh" . >/dev/null 2>&1); then ok "a module-level skip for an absent app is accepted (pytest exit 5)"; else bad "an author who skipped because the build does not exist yet was denied -- tzc18 iter 1"; fi
  if (cd "$W/noapp2" && bash "$OLDPWD/bin/check-derived-values.sh" . >/dev/null 2>&1); then ok "importing a module the build has not written yet is accepted"; else bad "an import of the not-yet-built module was held against the author"; fi
  if (cd "$W/badassert" && bash "$OLDPWD/bin/check-derived-values.sh" . >/dev/null 2>&1); then bad "a wrong module-level assertion (real collection error) was accepted"; else ok "a wrong module-level assertion is still denied at the author"; fi
else
...
if (cd "$W/helperok" && bash "$OLDPWD/bin/check-derived-values.sh" . >/dev/null 2>&1); then ok "a correct pin through a local helper is accepted"; else bad "a pin through a local helper was denied as a paste -- tzc19's eight denials"; fi
rc=0; out=$(cd "$W/helperbad" && bash "$OLDPWD/bin/check-derived-values.sh" . 2>&1) || rc=$?
...
if python3 -c 'import pytest' >/dev/null 2>&1; then
  rc=0; out=$(cd "$W/dup" && bash "$OLDPWD/bin/check-derived-values.sh" . 2>&1) || rc=$?
...
if (cd "$W/lexstub" && bash "$OLDPWD/bin/check-derived-values.sh" . >/dev/null 2>&1); then bad "a Lex test defining its own implementation was accepted -- lexwc4 iter 1"; else ok "a stub-only Lex test is denied where it was written"; fi
if (cd "$W/lexok" && bash "$OLDPWD/bin/check-derived-values.sh" . >/dev/null 2>&1); then ok "a Lex test that imports the build's module passes"; else bad "a Lex test importing the module was denied"; fi
