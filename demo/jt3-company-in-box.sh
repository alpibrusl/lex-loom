...
if [ -s "$OUTD/report.md" ] && (cd "$OUTD" && LOOM_SEARCH_LEDGER="$OUTD/ledger.txt" python3 "$JT_DIR/bin/check-research-report.sh" . > "$OUTD/host-check.txt" 2>&1); then ok "host-side re-check: $(command grep -o 'RESEARCH_REPORT_OK.*' "$OUTD/host-check.txt" | cut -c1-60)..."; else bad "host-side re-check refused the box's report: $(tail -3 "$OUTD/host-check.txt" 2>/dev/null | tr '\n' ' ' | cut -c1-200)"; fi
