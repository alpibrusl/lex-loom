...
| `[infra].hosting` / `.domain` | deploy target | declared-intent still (manifest fields aren't threaded yet) — but a real `deploy_hetzner` tool now exists (#101), driven directly by `HETZNER_HOST`/`HETZNER_USER`/`HETZNER_SSH_KEY`/`HETZNER_REMOTE_DIR` env vars a human sets, same pattern as `X402_*`. Setting `DEPLOY_DOMAIN` upgrades it to the deterministic TLS path (#188): `src/deploy_scaffold.lex` generates docker-compose.yml + a Caddyfile (automatic Let's Encrypt) as pure-function output, the remote command becomes `docker compose up -d --build`, and the health check hits `https://<domain>` so a green deploy proves the certificate provisioned |
...
| **`web-pwa`** | an installable Progressive Web App — the issue's "€100-viable mobile app": installable from a URL, no app store, no store fee. Same Node/TS stack as `node-ts-api` (node:* builtins, no npm install, no framework, no build step) plus the three installability pieces pre-wired: app-shell `index.html`, `manifest.webmanifest` (standalone display, icons), and a cache-first service worker for offline. `ts_build` writes the whole project through `ts_check` (TS syntax-checked, manifest parse-checked, css/html stored). |
| **`rn-expo-web`** | React Native code, exported to the WEB target — the first path of the **bootstrap-install weight class** (#256): its dependencies (expo, react-native-web) install exactly ONCE, at company bootstrap, via the skeleton's `.loom-install` (npm ci from the committed lockfile) — human-triggered, same trust level as the bootstrap's own `git init`/`gh repo create`, never during sprints. `npm run build` (`expo export --platform web`) produces `dist/`, served by a node:* builtins server with the standard `/loom/*` surface — same boot + curl launch story. `.tsx`/`.jsx` sources are stored unchecked by `ts_check` (no builtin JSX checker) — instead, the **sprint↔workspace bridge** gates them: on every ts_build node of a company whose workspace carries the `.loom-installed` marker, the `spec compiles` gate overlays the sprint's work dir onto a scratch copy of the workspace (node_modules symlinked in) and runs the real `npm run build` (expo export); bundler errors flow back into the agent's repair loop, and a marker with a missing install is a hard gate failure, never a silent skip. Native→App Store remains a separate, human-gated, out-of-scope track. |
| **`nextjs`** | Next.js (app router, `output: "standalone"`) — the second path of the bootstrap-install weight class: `.loom-install` runs `npm ci` once at bootstrap from the committed lockfile; `npm run build` (`next build`) emits `.next/standalone/server.js`, launched with PLAIN node + `PORT` env (no node_modules at runtime, same cheap launch story). The standard `/health` + `/loom/*` surface is implemented as route handlers inside the app, and the landing page is genuinely server-rendered — the reason to pick this over `web-pwa`. The sprint↔workspace bridge gates it with zero new code (marker-keyed `npm run build`); Turbopack is why the bridge hardlink-farms node_modules instead of symlinking. |
