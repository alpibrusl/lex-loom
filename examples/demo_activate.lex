...
fn demo_skip() -> [env, io, time, crypto, random, sql, fs_read, fs_write, net, concurrent, llm, proc, vcs] Unit {
  match open_db("/tmp/demo-activate.db") {
    ...
    Ok(db) => {
      let ctx1 := { idx: 1, last_verdict: "", digest_summary: "", accepted_count: 0, bounced_count: 0 }
      let cfg := { id: "demo-activate", request: "r", model: "none", db: db, api_calls_max: 10, roster: cast.empty_roster(), trail_log: None, review_transitions: false, depth: 0, iter_ctx: Some(ctx1) }
