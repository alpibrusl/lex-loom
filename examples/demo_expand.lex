...
fn demo_depth_cap() -> [env, io, time, crypto, random, sql, fs_read, fs_write, net, concurrent, llm, proc, vcs, approval] Unit {
...
fn demo_live_child() -> [env, io, time, crypto, random, sql, fs_read, fs_write, net, concurrent, llm, proc, vcs, approval] Unit {
  let db_path := get_env("DB_PATH", "/tmp/demo-expand.db")
  match open_db(db_path) {
    Err(e) => io.print(str.concat("FATAL: ", e)),
    Ok(db) => {
