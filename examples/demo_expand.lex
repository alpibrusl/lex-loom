...
fn demo_depth_cap() -> [env, io, time, crypto, random, sql, fs_read, fs_write, net, concurrent, llm, proc, vcs] Unit {
  ...
  match open_db(db_path) {
    ...
    Ok(db) => {
      let cfg := { id: "demo-depthcap", request: "parent request", model: get_env("MODEL", "gemma4:latest"), db: db, api_calls_max: 50, roster: cast.empty_roster(), trail_log: None, review_transitions: false, depth: 3 }
      let node := { id: "expander", role: "build", gate: "spec json", expand: Some("Decompose and build the sub-feature") }
...
fn demo_live_child() -> [env, io, time, crypto, random, sql, fs_read, fs_write, net, concurrent, llm, proc, vcs] Unit {
  ...
  match open_db(db_path) {
    ...
    Ok(db) => {
      ...
      let cfg := { id: "demo-live", request: "Build a tiny math utility", model: get_env("MODEL", "qwen3-coder:30b"), db: db, api_calls_max: 60, roster: cast.empty_roster(), trail_log: None, review_transitions: false, depth: 0 }
      let node := { id: "math_feature", role: "build", gate: "spec json", expand: Some("Write a pure Lex function add(a :: Int, b :: Int) -> Int that returns a + b, with an examples block.") }
