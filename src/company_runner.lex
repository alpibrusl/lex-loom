...
fn run_iterations(db :: conn.ConnDb, ccfg :: company.CompanyCfg, k :: Int, parent_sprint :: Str, api_max :: Int) -> [env, io, time, crypto, random, sql, fs_read, fs_write, net, concurrent, llm, proc, vcs] CompanyRunResult {
  ...
  let scfg := { id: sprint_id, request: ccfg.goal, model: ccfg.model, db: db, api_calls_max: api_max, roster: cast.empty_roster(), trail_log: trail_none, review_transitions: false, depth: 0 }
  ...
  if stop {
    ...
    if k >= ccfg.max_iterations {
      ...
      run_iterations(db, ccfg, k + 1, sprint_id, api_max)
    }
  }
}
...
fn run_company(db :: conn.ConnDb, ccfg :: company.CompanyCfg, api_max :: Int) -> [env, io, time, crypto, random, sql, fs_read, fs_write, net, concurrent, llm, proc, vcs] CompanyRunResult {
  ...
  let res := run_iterations(db, ccfg, 1, "", api_max)
