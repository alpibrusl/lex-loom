...
type NodeDiff =
  ...
  | NodeRemoved(Str)                    # removed node id
  | NodeChanged(graph.Node, graph.Node) # (old, new)

type EdgeDiff =
  | EdgeAdded(graph.Edge)
  | EdgeRemoved(graph.Edge)
  | EdgeModified(graph.Edge, graph.Edge) # (old, new)
...
fn nodes_equal(a :: graph.Node, b :: graph.Node) -> Bool {
  a.id == b.id && a.role == b.role && a.gate == b.gate
}

fn edges_equal(a :: graph.Edge, b :: graph.Edge) -> Bool {
  a.from == b.from && a.to == b.to && a.handoff == b.handoff
}
...
fn find_node(nodes :: List[graph.Node], id :: Str) -> Option[graph.Node] {
  list.fold(nodes, None, fn (acc :: Option[graph.Node], n :: graph.Node) -> Option[graph.Node] {
    match acc {
      Some(_) => acc,
      None    => if n.id == id { Some(n) } else { None },
    }
  })
}
...
fn find_edge(edges :: List[graph.Edge], from :: Str, to :: Str) -> Option[graph.Edge] {
  list.fold(edges, None, fn (acc :: Option[graph.Edge], e :: graph.Edge) -> Option[graph.Edge] {
    match acc {
      Some(_) => acc,
      None    => if e.from == from && e.to == to { Some(e) } else { None },
    }
  })
}
...
fn diff(old :: graph.SprintGraph, new :: graph.SprintGraph) -> GraphDiff {
  {
    node_diffs: diff_nodes(old.nodes, new.nodes),
    edge_diffs: diff_edges(old.edges, new.edges),
  }
}
...
fn is_empty(d :: GraphDiff) -> Bool {
  list.is_empty(d.node_diffs) && list.is_empty(d.edge_diffs)
}
...
fn nodes_to_rerun(d :: GraphDiff, g :: graph.SprintGraph) -> List[Str] {
  ...
  # Downstream of modified edges
  let from_edges := list.fold(d.edge_diffs, [], fn (acc :: List[Str], ed :: EdgeDiff) -> List[Str] {
    match ed {
      EdgeAdded(e)      => if graph.str_contains(acc, e.to) { acc } else { list.concat(acc, [e.to]) },
      EdgeModified(_, e) => if graph.str_contains(acc, e.to) { acc } else { list.concat(acc, [e.to]) },
      EdgeRemoved(_)    => acc,
    }
  })
  # Union, deduplicated
  list.fold(from_edges, direct, fn (acc :: List[Str], id :: Str) -> List[Str] {
    if graph.str_contains(acc, id) { acc } else { list.concat(acc, [id]) }
  })
}
...
fn nodes_to_keep(d :: GraphDiff, g :: graph.SprintGraph) -> List[Str] {
  ...
  graph.str_filter(graph.node_ids(g), fn (id :: Str) -> Bool {
    !graph.str_contains(rerun, id)
  })
}

examples {
  # Identical graphs produce empty diff
  let g := {
    id: "g", phase: graph.Intake,
    nodes: [{ id: "a", role: "build", gate: "spec true" }],
    edges: [],
  }
  is_empty(diff(g, g)) == true

  # Added node appears in diff
  let g1 := { id: "g", phase: graph.Intake, nodes: [{ id: "a", role: "build", gate: "spec true" }], edges: [] }
  let g2 := {
    id: "g", phase: graph.Intake,
    nodes: [
  ...
  list.is_empty(diff(g1, g2).node_diffs) == false

  # Changed gate triggers re-run
  let g3 := { id: "g", phase: graph.Intake, nodes: [{ id: "a", role: "build", gate: "spec true" }],         edges: [] }
  let g4 := { id: "g", phase: graph.Intake, nodes: [{ id: "a", role: "build", gate: "spec output != ''" }], edges: [] }
  list.is_empty(nodes_to_rerun(diff(g3, g4), g4)) == false
}
