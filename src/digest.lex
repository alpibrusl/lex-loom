...
type TightenedSpec = {
  node_role :: Str,
  spec_src  :: Str,   # new/updated gate spec source
  reason    :: Str,   # why it was tightened (trail evidence)
}

# The full Digest artifact produced by the Scribe at end of sprint.
type DigestArtifacts = {
  sprint_id        :: Str,
  lessons          :: Str,
  tightened_specs  :: List[TightenedSpec],
  seed_graph       :: graph.SprintGraph,
}

type DigestResult =
  | DigestOk(DigestArtifacts)
  | DigestFailed(Str)
...
fn load_trail(db :: Db, sprint_id :: Str) -> [sql, fs_read] List[TrailRow] {
  let q := str.join([
    "SELECT event_kind, data_json, ts FROM traces WHERE agent_id='",
    sq(sprint_id), "' ORDER BY ts"
  ], "")
...
fn save_tightened_spec(db :: Db, sprint_id :: Str, ts :: TightenedSpec) -> [sql, fs_write, time, random, crypto] Result[Unit, Str] {
  ...
  let q := str.join([
    "INSERT INTO tightened_specs (id, sprint_id, node_role, spec_src, reason, created_at) VALUES ('",
    sq(id), "', '", sq(sprint_id), "', '", sq(ts.node_role), "', '",
    sq(ts.spec_src), "', '", sq(ts.reason), "', '", now, "')"
  ], "")
  match sql.exec(db, q, []) {
    Err(e) => Err(e.message),
    Ok(_)  => Ok(()),
...
fn save_digest(db :: Db, d :: DigestArtifacts) -> [sql, fs_write, time, random, crypto] Result[Unit, Str] {
  ...
  let q := str.join([
    "INSERT INTO digests (id, sprint_id, lessons, seed_graph_json, created_at) VALUES ('",
    sq(id), "', '", sq(d.sprint_id), "', '", sq(d.lessons), "', '", sq(seed_j), "', '", now, "')"
  ], "")
  match sql.exec(db, q, []) {
    Err(e) => Err(e),
    Ok(_) => {
      ...
      let first_err := list.fold(spec_results, None, fn (acc :: Option[Str], r :: Result[Unit, Str]) -> Option[Str] {
        match acc {
          Some(_) => acc,
          None    => match r { Err(e) => Some(e), Ok(_) => None },
        }
      })
      match first_err {
        Some(e) => Err(e),
...
fn load_tightened_specs(db :: Db, sprint_id :: Str) -> [sql, fs_read] List[TightenedSpec] {
  ...
  let q := str.join([
    "SELECT node_role, spec_src, reason FROM tightened_specs WHERE sprint_id='",
    sq(sprint_id), "' ORDER BY created_at"
  ], "")
...
fn load_seed_graph(db :: Db, sprint_id :: Str) -> [sql, fs_read] Option[graph.SprintGraph] {
  ...
  let q := str.join([
    "SELECT seed_graph_json FROM digests WHERE sprint_id='",
...
fn parse_tightened_spec(j :: jv.Json) -> Option[TightenedSpec] {
  let role := match jv.get_field(j, "node_role") { Some(JStr(s)) => s, _ => "" }
  let src  := match jv.get_field(j, "spec_src")  { Some(JStr(s)) => s, _ => "" }
  let why  := match jv.get_field(j, "reason")    { Some(JStr(s)) => s, _ => "" }
  if str.is_empty(role) || str.is_empty(src) {
    None
  } else {
...
fn default_seed_graph(next_sprint_id :: Str) -> graph.SprintGraph {
  {
    id:    str.concat(next_sprint_id, "-seed"),
    phase: graph.Intake,
    nodes: [
      { id: "intake",  role: "architect", gate: "spec non-empty" },
      { id: "build",   role: "build",     gate: "spec non-empty" },
      { id: "qa",      role: "qa",        gate: "spec non-empty" },
      { id: "demo",    role: "demo",      gate: "spec non-empty" },
    ],
    edges: [
      { from: "intake", to: "build",  handoff: "schema {}" },
      { from: "build",  to: "qa",     handoff: "schema {}" },
      { from: "qa",     to: "demo",   handoff: "schema {}" },
    ],
  }
}
...
fn scribe_prompt(sprint_id :: Str, trail_text :: Str, next_sprint_id :: Str) -> Str {
  str.join([
    "You are the Scribe. Sprint `", sprint_id, "` has completed.\n\n",
    "Sprint trail:\n```\n", trail_text, "\n```\n\n",
    ...
    "  }\n",
    "}\n\n",
    "Rules for seed_graph: must be a valid SprintGraph — no cycles, every demo node must ",
    "have a qa ancestor, every node needs a non-empty gate.\n",
...
fn run_digest(
  ...
  model          :: Str,
  db             :: Db,
) -> [io, time, crypto, random, sql, fs_read, fs_write, net, concurrent, llm, proc] DigestResult {
  ...
  let __tt := trace.record(db, sprint_id, sprint_id, "digest_produced",
    str.join(["{\"output_len\":", int.to_str(str.len(output)), "}"], ""))
  ...
  match save_digest(db, artifacts) {
    Err(e) => {
      let __te := trace.record(db, sprint_id, sprint_id, "digest_save_failed",
        str.join(["{\"error\":\"", e, "\"}"], ""))
      DigestFailed(e)
    },
    Ok(_) => {
      let __tv := trace.record(db, sprint_id, sprint_id, "digest_saved",
        str.join(["{\"specs\":", int.to_str(list.len(artifacts.tightened_specs)),
                  ",\"seed_nodes\":", int.to_str(list.len(artifacts.seed_graph.nodes)), "}"], ""))
      DigestOk(artifacts)
    },
  }
}
...
fn specs_context(specs :: List[TightenedSpec]) -> Str {
  if list.is_empty(specs) {
    ...
    str.join([
      ...
      list.fold(lines, "", fn (acc :: Str, l :: Str) -> Str { str.concat(acc, str.concat(l, "\n")) }),
    ], "")
  }
}
