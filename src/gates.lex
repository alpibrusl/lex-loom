...
fn from_spec_verdict(v :: sp.Verdict) -> GateVerdict {
  match v {
    sp.Allow           => GateAllow,
    sp.Deny(reason)    => GateDeny(reason),
    sp.Inconclusive(r) => GateDeny(str.concat("inconclusive (treated as deny): ", r)),
  }
}
...
fn spec_non_empty() -> sp.Spec {
  {
    name: "non-empty",
    quantifiers: [sp.QStr("output")],
    predicate: sp.EBinop({ op: sp.op_neq(), lhs: sp.EVar("output"), rhs: sp.EConst(sp.VStr("")) }),
  }
}

fn spec_len_gt(n :: Int) -> sp.Spec {
  # No string-length operator in lex-spec — approximate via non-empty + custom check
  # This is handled by the custom evaluator below, not via sp.Spec
  spec_non_empty()
}
            ...
            if trimmed == "spec json" {
              ...
              if str.starts_with(trimmed, "spec json-field ") {
                ...
                if str.starts_with(trimmed, "spec len-gt ") {
                  ...
                  # Unknown gate — fall back to non-empty (safe default)
                  from_spec_verdict(ev.eval(spec_non_empty(), [("output", sp.VStr(output))]))
                }
              }
            }
