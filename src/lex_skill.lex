...
fn guide_http() -> Str {
  str.join([
    "HTTP SERVER PATTERN (std.net)",
    "",
    "Two entry points:",
    "  net.serve(port, \"handle\")      -- handler fn referenced by name (string)",
    "  net.serve_fn(port, handler_fn) -- handler fn passed directly",
    "",
    "Request and Response types (declare these at the top of your file):",
    "  type Request  = { body :: Str, method :: Str, path :: Str, query :: Str }",
    "  type Response = { body :: Str, status :: Int }",
    "",
    "For streaming responses add a headers field:",
    "  type Response = { body :: ResponseBody, status :: Int, headers :: Map[Str, Str] }",
    "  -- ResponseBody variants: BodyStr(Str) | BodyStream(Iter[Str]) | BodyBytes(Iter[List[Int]])",
    "  -- Import: import \"std.map\" as map  then  map.from_list([(\"content-type\",\"application/json\")])",
    "",
    "ROUTING — match on method then path:",
    "  fn handle(req :: Request) -> [net, time, io] Response {",
    "    match req.method {",
    "      \"GET\" => match req.path {",
    "        \"/health\" => { status: 200, body: \"{\\\"ok\\\":true}\" },",
    "        _ => match str.strip_prefix(req.path, \"/items/\") {",
    "          Some(id) => get_item(id),",
    "          None     => { status: 404, body: \"{\\\"error\\\":\\\"not found\\\"}\" },",
    "        },",
    "      },",
    "      \"POST\" => match req.path {",
    "        \"/items\" => create_item(req.body),",
    ...
    "",
    "EFFECT DISCIPLINE for HTTP:",
    "  - Pure routes: no effect annotation on the route fn, but handle() must declare effects",
    "  - Route calling net.get: needs [net]",
    "  - Route calling io.read/write: needs [io] or [fs_read]/[fs_write]",
    "  - handle()'s effect row = union of all route effects",
    "  - main() always needs at least [net] for net.serve",
    "",
    "IMPORTS for HTTP:",
    "  import \"std.net\" as net",
    "  import \"std.str\" as str",
    "  import \"std.int\" as int",
    "  import \"std.map\" as map   # only if using headers/Map",
    "  import \"std.iter\" as iter  # only if streaming",
  ], "\n")
}
...
fn guide_mcp() -> Str {
  str.join([
    "MCP SERVER PATTERN (lex-mcp)",
    "",
    "lex-mcp provides THREE transports from ONE AgentDef:",
    "",
    "1. STDIO (Claude Desktop, local tools):",
    "   import \"lex-mcp/src/mcp\" as mcp",
    "   fn main() -> [io, time, ...] Nil { mcp.server.run(my_agent) }",
    "",
    "2. HTTP (remote MCP clients / lex-mcp-client):",
    "   import \"lex-mcp/src/http\" as mcph",
    "   fn main() -> [net, io, ...] Nil { mcph.run_http(my_agent, 8080) }",
    "",
    "3. DUAL — A2A on / and MCP on /mcp, same port (RECOMMENDED):",
    "   import \"lex-mcp/src/compose\" as compose",
    "   fn main() -> [net, io, ...] Nil { compose.serve_both(my_agent, 8080) }",
    "   -- GET  /.well-known/agent.json  → A2A AgentCard",
    "   -- POST /                         → A2A JSON-RPC dispatch",
    "   -- POST /mcp                      → MCP streamable-HTTP dispatch",
    "   Both surfaces derive from the same skills list — they never drift.",
    "",
    "BUILDING AN AGENTDEF:",
    ...
    "",
    "   fn my_capability() -> cap.Capability {",
    "     cap.inbound(\"tool_name\", \"What it does.\",",
    "       { title: \"Args\", description: \"Input schema\",",
    "         fields: [sch.required_str(\"input\", [])] })",
    "   }",
    "",
    "   fn my_handler(m :: msg.Message) -> [io, ...] srv.HandlerOutcome {",
    "     let reply := msg.agent_text(\"response here\")",
    "     { next_state: TSCompleted, reply: Some(reply), artifacts: [] }",
    "   }",
    "",
    "   fn make_agent() -> srv.AgentDef {",
    "     let c := card.make(\"my-agent\", \"Description.\", \"0.1.0\",",
    "                        \"http://localhost:8080\", [my_capability()])",
    "     srv.make_agent_def(c, [{ capability: my_capability(), handle: my_handler }])",
    "   }",
    "",
    "EXTRACTING TEXT from a Message (DataPart arrives from MCP tools/call):",
    ...
    "             Some(v) => match jv.as_str(v) { Some(s) => s, None => acc },",
    "             None => acc,",
    "           },",
    "           _ => acc,",
    "         }",
    ...
    "   }",
    "",
    "EFFECT ROW for MCP/A2A servers:",
    "   [io, time, crypto, random, sql, fs_read, fs_write, net, concurrent, llm, proc]",
    "   (use this full row on main() and handler fns to avoid effect mismatch errors)",
    "",
    "IMPORTS for MCP:",
    "  import \"lex-schema/schema\" as sch",
    "  import \"lex-schema/json_value\" as jv",
...
fn guide_agent() -> Str {
  str.join([
    "LLM AGENT PATTERN (lex-llm + lex-agent-llm)",
    "",
    "lex-llm provides a multi-step tool-call loop. Provider is selected from env vars",
    "(LITELLM_BASE_URL → LiteLLM proxy, ANTHROPIC_API_KEY → Anthropic, etc.).",
    "",
    "PROVIDER SELECTION (from roles.lex — use the same pattern):",
    "  import \"lex-llm/src/providers\" as providers",
    "  -- providers.litellm()            # via LITELLM_BASE_URL",
    "  -- providers.anthropic()          # via ANTHROPIC_API_KEY",
    "  -- providers.ollama_at(url)       # local Ollama",
    "  -- providers.mlx_at(url)          # Apple Silicon MLX",
    "",
    "DEFINING AN AGENT:",
    ...
    "  let agent := {",
    "    name: \"my-agent\",",
    "    goal: \"System prompt here.\",",
    "    model: prov.make_model_ref(\"litellm\", \"qwen3-coder:30b\"),",
    ...
    "",
    "RUNNING THE LOOP:",
    "  let messages := [msg.user(\"task description here\")]",
    "  let step_iter := ag.run_loop(agent, messages)",
    "  -- step_iter yields: TextChunk(s) | ToolCall(...) | StepDone(result)",
    "  -- Collect with iter.to_list(step_iter) or fold over it",
    "",
    "SUBPROCESS DISPATCH (agent_dispatcher pattern — proc effect):",
    "  import \"std.proc\" as proc",
    "  fn run_agent(cmd :: Str, args :: List[Str]) -> [proc] Result[Str, Str] {",
    "    match proc.spawn(cmd, args) {",
    "      Ok(r)  => Ok(r.stdout),",
    "      Err(e) => Err(e),",
    "    }",
    "  }",
    "  -- Requires --allow-proc <binary> at runtime",
    "",
    "WRAPPING AN LLM LOOP AS AN MCP/A2A SKILL (lex-agent-llm):",
    "  import \"lex-agent-llm/src/bridge\" as bridge",
    "  let skill := bridge.skill_of_loop(agent_def, loop_fn)",
    "",
    "IMPORTS for agent tasks:",
    "  import \"lex-llm/src/agent\" as ag",
    "  import \"lex-llm/src/provider\" as prov",
    "  import \"lex-llm/src/providers\" as providers",
    "  import \"lex-llm/src/tool\" as t",
...
fn guide_sql() -> Str {
  str.join([
    "SQL / PERSISTENCE PATTERN (std.sql + lex-trail)",
    "",
    "std.sql — SQLite (and Postgres) query builder:",
    "  import \"std.sql\" as sql",
    "",
    "  fn open_db(path :: Str) -> [sql] Result[sql.Db, Str] { sql.open(path) }",
    "",
    "  -- Execute DDL / DML (no rows returned):",
    "  sql.exec(db, \"CREATE TABLE IF NOT EXISTS items (id TEXT, data TEXT)\", [])",
    "  sql.exec(db, \"INSERT INTO items VALUES (?, ?)\", [sql.str(id), sql.str(data)])",
    "",
    "  -- Query returning rows:",
    "  match sql.query(db, \"SELECT id, data FROM items WHERE id = ?\", [sql.str(id)]) {",
    "    Ok(rows) => ...,  # rows :: List[List[sql.Value]]",
    "    Err(e)   => ...,",
    "  }",
    "",
    "  -- Extract values from a row:",
    "  -- sql.Value variants: VStr(Str) | VInt(Int) | VFloat(Float) | VNull | VBool(Bool)",
    "  let val := match list.head(row) {",
    "    Some(VStr(s)) => s,",
    "    _             => \"\",",
    "  }",
    "",
    "  -- Parameter helpers: sql.str(s), sql.int(n), sql.float(f), sql.null()",
    "",
    "lex-trail — append-only content-addressed audit log:",
    "  import \"lex-trail/src/log\" as trail_log",
    "  import \"lex-trail/src/emit\" as emit",
    "",
    "  -- Open (ephemeral in-memory or persistent SQLite):",
    "  let log := trail_log.open_memory()           # ephemeral",
    "  let log := trail_log.open(\"/tmp/my.db\")     # persistent",
    "",
    "  -- Append an event:",
    "  let event_id := trail_log.append(log, emit.llm_step(agent_id, step_data))",
    "",
    "  -- Query events:",
    "  let events := trail_log.range(log, 0, 100)",
    "",
    "EFFECT for SQL:",
    "  Any function calling sql.* needs the [sql] effect.",
    "  Any function calling io.write / io.read needs [io] or [fs_write]/[fs_read].",
    "",
    "IMPORTS for SQL:",
    "  import \"std.sql\" as sql",
    "  import \"std.list\" as list",
...
fn guide_streaming() -> Str {
  str.join([
    "STREAMING HTTP PATTERN (std.net + std.iter)",
    "",
    "Use net.serve_fn (not net.serve) for streaming — it accepts a handler fn directly.",
    "",
    "ResponseBody variants for streaming:",
    "  BodyStr(Str)            -- eager string (non-streaming)",
    "  BodyStream(Iter[Str])   -- chunked text (SSE, line-by-line)",
    "  BodyBytes(Iter[List[Int]]) -- chunked binary",
    "",
    "Response type for streaming (must include headers field):",
    "  type Response = { body :: ResponseBody, status :: Int, headers :: Map[Str, Str] }",
    "",
    ...
    "      },",
    "      _ => { status: 404, body: BodyStr(\"not found\"),",
    "             headers: map.from_list([(\"content-type\", \"text/plain\")]) },",
    "    }",
    "  }",
    "  fn main() -> [net] Unit { net.serve_fn(8088, handler) }",
    "",
    "NOTE: BodyStream emits chunks under chunked transfer-encoding.",
    "      Content-Length is not set automatically — do not set it manually.",
  ], "\n")
}
