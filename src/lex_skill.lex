...
fn make_lex_guidelines_tool() -> t.Tool {
  let params := { title: "LexGuidelines", description: "Fetch the essential Lex language rules", fields: [s.required_str("topic", [])] }
  t.define("lex_guidelines", "Return the essential Lex syntax, effect, type, and stdlib rules. Lex is NOT in your training data — call this FIRST, before writing any Lex code.", params, fn (args :: jv.Json) -> [net, io, proc] Result[jv.Json, e.Errors] {
