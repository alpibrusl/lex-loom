...
# ORG2 (lex-loom#217): the delegation mailbox. An assignment is one role
# handing a TYPED task (closed vocabulary, delegation.lex) to a direct
...
fn ddl_assignments_idx() -> Str {
  "CREATE INDEX IF NOT EXISTS idx_assignments_company ON assignments(company_id, status)"
}

fn ddl_mission_ledger() -> Str {
  "CREATE TABLE IF NOT EXISTS mission_ledger (id TEXT PRIMARY KEY, company_id TEXT NOT NULL, idx INTEGER NOT NULL, mission TEXT NOT NULL, source TEXT NOT NULL DEFAULT '', approved_by TEXT NOT NULL DEFAULT '', created_at TEXT NOT NULL)"
}

fn ddl_mission_ledger_idx() -> Str {
  "CREATE INDEX IF NOT EXISTS idx_mission_ledger_company ON mission_ledger(company_id, idx)"
}

fn ddl_role_defs() -> Str {
  "CREATE TABLE IF NOT EXISTS role_defs (id TEXT PRIMARY KEY, company_id TEXT NOT NULL, kind TEXT NOT NULL, system_prompt TEXT NOT NULL, tool_profile TEXT NOT NULL DEFAULT 'none', grant_preset TEXT NOT NULL DEFAULT 'Demo', status TEXT NOT NULL DEFAULT 'proposed', proposed_by TEXT NOT NULL DEFAULT '', approved_by TEXT NOT NULL DEFAULT '', attention_id TEXT NOT NULL DEFAULT '', created_at TEXT NOT NULL, updated_at TEXT NOT NULL DEFAULT '')"
}

fn ddl_role_defs_idx() -> Str {
...
fn ddl_allocations() -> Str {
  "CREATE TABLE IF NOT EXISTS allocations (id TEXT PRIMARY KEY, company_id TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'proposed', changes_json TEXT NOT NULL DEFAULT '[]', rationale TEXT NOT NULL DEFAULT '', predicted_target_cents INTEGER NOT NULL DEFAULT 0, predicted_by_iter INTEGER NOT NULL DEFAULT 0, attention_id TEXT NOT NULL DEFAULT '', proposed_at_iter INTEGER NOT NULL DEFAULT 0, grade TEXT NOT NULL DEFAULT '', created_at TEXT NOT NULL, updated_at TEXT NOT NULL DEFAULT '')"
}

fn ddl_allocations_idx() -> Str {
  "CREATE INDEX IF NOT EXISTS idx_allocations_company ON allocations(company_id, status)"
}
...
# HB2 (#214): the append-only external-event ledger. Inserted, never edited,
# never deleted; the only mutation anywhere is the one-shot consumption mark
# (events.lex `consume_all`). There is deliberately no updated_at column —
# a row's whole lifecycle is created → (optionally) consumed.
# Named company_events, NOT events: lex-trail's schema owns a table called
# `events` (hash-chained trail rows, incompatible columns) and the settlement
# path opens a lex-trail log on the SAME connection loom migrates — the exact
# collision operate_sweep already documents for `attestations`.
fn ddl_events() -> Str {
  "CREATE TABLE IF NOT EXISTS company_events (id TEXT PRIMARY KEY, company_id TEXT NOT NULL, kind TEXT NOT NULL, source TEXT NOT NULL DEFAULT '', body_json TEXT NOT NULL DEFAULT '', created_at TEXT NOT NULL, consumed_at TEXT NOT NULL DEFAULT '', consumed_by TEXT NOT NULL DEFAULT '')"
}

# Capabilities a company sells (#398, piece 3): one row per offer, read back as
# lex-economy CompanyOffers so capability.find can match buyers to suppliers
# across the companies sharing this database.
fn ddl_capability_offers() -> Str {
  "CREATE TABLE IF NOT EXISTS capability_offers (company_id TEXT NOT NULL, capability TEXT NOT NULL, attrs_json TEXT NOT NULL DEFAULT '{}', created_at TEXT NOT NULL, PRIMARY KEY (company_id, capability))"
}

fn ddl_events_idx() -> Str {
  "CREATE INDEX IF NOT EXISTS idx_company_events ON company_events(company_id, consumed_at)"
}
