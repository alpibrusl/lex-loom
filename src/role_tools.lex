...
fn tools_for(role :: Str) -> List[Str] {
  if role == "py_test_author" {
    ...
    if role == "ts_test_author" {
      ...
      if role == "test_author" {
        ["lex_guidelines", "lex_docs", "lex_check"]
      } else {
        if role == "build" {
          ["lex_guidelines", "lex_docs", "lex_check", "read_file"]
        } else {
          if role == "py_build" {
            ["py_check", "read_file"]
          } else {
                            ...
                            if role == "security" {
                              ...
                              if role == "content_creator" {
                                ...
                                if role == "cx" {
                                  ...
                                  if role == "research" or role == "opportunity_research" or role == "community" {
                                    ["web_search"]
                                  } else {
                                    if role == "founder" {
                                      []
