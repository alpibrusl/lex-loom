...
fn skill_start(db :: Db, model_default :: Str) -> srv.Skill {
  let params := {
    title: "StartSprint", description: "Start a new sprint",
    fields: [
      s.required_str("sprint_id", []),
      s.required_str("request",   []),
      s.optional(s.required_str("model", [])),
    ],
  }
  {
    capability: cap.inbound("start", "Start a new loom sprint given a project request.", params),
    handle: fn (m :: msg.Message) -> [io, time, crypto, random, sql, fs_read, fs_write, net, concurrent, llm, proc] srv.HandlerOutcome {
      ...
      if str.is_empty(sprint_id) || str.is_empty(request) {
...
fn skill_status(db :: Db) -> srv.Skill {
  let params := {
    title: "SprintStatus", description: "Get sprint status",
    fields: [s.required_str("sprint_id", [])],
  }
  {
    capability: cap.inbound("status", "Return the current state of a sprint.", params),
    handle: fn (m :: msg.Message) -> [io, time, crypto, random, sql, fs_read, fs_write, net, concurrent, llm, proc] srv.HandlerOutcome {
      let args      := msg_to_json(m)
      let sprint_id := str_field(args, "sprint_id")
      if str.is_empty(sprint_id) {
        ...
        match tenant.find(db, sprint_id) {
          ...
          Some(r) => ok_outcome(jv.stringify(JObj([
            ("sprint_id", JStr(sprint_id)),
...
fn skill_trail(db :: Db) -> srv.Skill {
  let params := {
    title: "SprintTrail", description: "Fetch sprint trail events",
    fields: [
      s.required_str("sprint_id", []),
  ...
  {
    capability: cap.inbound("trail", "Return the last N trail events for a sprint.", params),
    handle: fn (m :: msg.Message) -> [io, time, crypto, random, sql, fs_read, fs_write, net, concurrent, llm, proc] srv.HandlerOutcome {
      let args      := msg_to_json(m)
      let sprint_id := str_field(args, "sprint_id")
      if str.is_empty(sprint_id) {
...
fn skill_digest(db :: Db) -> srv.Skill {
  let params := {
    title: "SprintDigest", description: "Fetch Digest artifacts",
    fields: [s.required_str("sprint_id", [])],
  }
  {
    capability: cap.inbound("digest", "Return Digest lessons and tightened spec count.", params),
    handle: fn (m :: msg.Message) -> [io, time, crypto, random, sql, fs_read, fs_write, net, concurrent, llm, proc] srv.HandlerOutcome {
      let args      := msg_to_json(m)
      let sprint_id := str_field(args, "sprint_id")
      if str.is_empty(sprint_id) {
        ...
        ok_outcome(jv.stringify(JObj([
          ("sprint_id",       JStr(sprint_id)),
          ("tightened_specs", JInt(list.len(specs))),
          ("seed_graph",      seed_json),
        ])))
      }
    },
  }
...
fn make_agent(db :: Db, model_default :: Str, base_url :: Str) -> srv.AgentDef {
  let skills := [
    skill_start(db, model_default),
    skill_status(db),
    skill_trail(db),
    skill_digest(db),
  ]
  let skill_caps := list.map(skills, fn (sk :: srv.Skill) -> cap.Capability { sk.capability })
  let agent_card := card.make(
    "loom",
    "Multi-agent sprint cycle orchestrator",
    "0.5.0",
    base_url,
    skill_caps,
  )
...
fn str_field_or(j :: jv.Json, key :: Str, default :: Str) -> Str {
  match jv.get_field(j, key) {
    Some(JStr(s)) => if str.is_empty(s) { default } else { s },
    _             => default,
  }
}
