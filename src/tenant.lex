...
fn register(db :: Db, sprint_id :: Str, request :: Str, inbox_url :: Str) -> [sql, fs_write, time] Result[Unit, Str] {
...
fn complete(db :: Db, sprint_id :: Str) -> [sql, fs_write, time] Result[Unit, Str] {
  registry.set_status(db, sprint_id, "complete")
}

# Mark the sprint as failed.
fn fail(db :: Db, sprint_id :: Str) -> [sql, fs_write, time] Result[Unit, Str] {
  registry.set_status(db, sprint_id, "failed")
}

# Heartbeat — called periodically by run_sprint to signal liveness.
fn heartbeat(db :: Db, sprint_id :: Str) -> [sql, fs_write, time] Result[Unit, Str] {
