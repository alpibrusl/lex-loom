...
fn node_queue() -> Str { "loom:node" }
fn node_handler() -> Str { "invoke" }
...
fn node_job_payload(sprint_id :: Str, node_id :: Str, phase :: Str, input_ref :: Str, model :: Str) -> Str {
  jv.stringify(JObj([
    ("sprint_id", JStr(sprint_id)),
    ("node_id",   JStr(node_id)),
    ("phase",     JStr(phase)),
    ("input_ref", JStr(input_ref)),
    ("model",     JStr(model)),
  ]))
}
...
fn write_node_result(db :: Db, sprint_id :: Str, node_id :: Str, phase :: Str, accepted :: Bool, artifact :: Str, reason :: Str) -> [sql, fs_write, time, random, crypto] Result[Unit, Str] {
  ...
  let q := str.join([
    "INSERT INTO node_results (id, sprint_id, node_id, phase, accepted, artifact, reason, created_at) VALUES ('",
    sq(id), "', '", sq(sprint_id), "', '", sq(node_id), "', '", sq(phase), "', ",
    acc, ", '", sq(artifact), "', '", sq(reason), "', '", now, "')"
  ], "")
  match sql.exec(db, q, []) {
    Err(e) => Err(e.message),
    Ok(_)  => Ok(()),
...
fn read_node_results(db :: Db, sprint_id :: Str, phase :: Str) -> [sql, fs_read] List[NodeResultRow] {
  let q := str.join([
    "SELECT node_id, accepted, artifact, reason FROM node_results WHERE sprint_id='",
    sq(sprint_id), "' AND phase='", sq(phase), "'"
  ], "")
...
fn await_node_results(
  ...
  timeout_ms :: Int,
  poll_ms    :: Int,
) -> [sql, fs_read, time, io] Result[List[NodeResultRow], Str] {
...
fn make_worker_dispatch(
  db        :: Db,
  invoke_fn :: (Str, Str, Str, Str, Str) -> [io, time, crypto, random, sql, fs_read, fs_write, net, concurrent, llm, proc] { accepted :: Bool, artifact :: Str, reason :: Str },
) -> (Str, Str) -> [io, time, crypto, random, sql, fs_read, fs_write, net, concurrent] jobs.WorkOutcome {
  fn (handler :: Str, payload :: Str) -> [io, time, crypto, random, sql, fs_read, fs_write, net, concurrent] jobs.WorkOutcome {
    match handler {
      "invoke" => {
        let sprint_id := match jv.parse(payload) {
          Err(_) => "",
          Ok(j)  => match jv.get_field(j, "sprint_id") { Some(JStr(s)) => s, _ => "" },
        }
        let node_id   := match jv.parse(payload) {
          Err(_) => "",
          Ok(j)  => match jv.get_field(j, "node_id") { Some(JStr(s)) => s, _ => "" },
        }
        let phase     := match jv.parse(payload) {
          Err(_) => "",
          Ok(j)  => match jv.get_field(j, "phase") { Some(JStr(s)) => s, _ => "" },
        }
        let input_ref := match jv.parse(payload) {
          Err(_) => "",
          Ok(j)  => match jv.get_field(j, "input_ref") { Some(JStr(s)) => s, _ => "" },
        }
        let model := match jv.parse(payload) {
          Err(_) => "",
          Ok(j)  => match jv.get_field(j, "model") { Some(JStr(s)) => s, _ => "" },
        }
...
fn save_sprint_graph(db :: Db, sprint_id :: Str, phase :: Str, graph_json :: Str) -> [sql, fs_write, time, random, crypto] Result[Unit, Str] {
  ...
  let now := time.now_str()
  let q := str.join([
    "INSERT INTO sprint_graphs (id, sprint_id, phase, graph_json, created_at) VALUES ('",
    sq(id), "', '", sq(sprint_id), "', '", sq(phase), "', '", sq(graph_json), "', '", now, "')"
  ], "")
  match sql.exec(db, q, []) {
    Err(e) => Err(e.message),
    Ok(_)  => Ok(()),
