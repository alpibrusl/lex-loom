...
# #386: a file whose first line is its own name is refused, naming the line.
fn test_py_check_refuses_a_filename_as_first_line() -> [env, io, net, proc, random, fs_write] Result[Unit, Str] {
  let sprint := str.concat("t-fl/", crypto.random_str_hex(4))
  let tool := lexskill.make_py_check_tool("/tmp/loom-lexcheck-evidence-test.json", sprint)
  let bad := tool.execute(JObj([("filename", JStr("test_convert.py")), ("code", JStr("test_convert.py\nimport pytest\n\ndef test_x():\n    assert 1\n"))]))
  let good := tool.execute(JObj([("filename", JStr("test_ok.py")), ("code", JStr("# test_ok.py\nimport pytest\n\ndef test_x():\n    assert 1\n"))]))
  let __rm := proc.run("bash", ["-c", str.join(["rm -rf '", lexskill.py_work_dir(sprint), "'"], "")])
  if read_field(bad, "ok") == "false" and str.contains(read_field(bad, "output"), "first line is a bare file name") and read_field(good, "ok") == "true" {
    Ok(())
  } else {
    Err(str.join(["a file starting with its own name was written (ok=", read_field(bad, "ok"), ") or a commented name was refused (ok=", read_field(good, "ok"), ") -- tzc23's two collection errors"], ""))
  }
}
