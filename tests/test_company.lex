...
# #396: a sprint persists one sprint_graphs row per phase and subgraph; the
# build-status signal read only the last-written row (the Intake graph in
# lexwc2) and told the strategist no Lex build had ever been accepted a
# minute after one was. Every row must be read.
fn test_build_status_sees_a_build_in_any_graph_row() -> [sql, fs_write, time, crypto, random] Result[Unit, Str] {
  match conn.open(str.join(["/tmp/loom-t-", crypto.random_str_hex(8), ".db"], "")) {
    Err(_) => Err("open db failed"),
    Ok(db) => match migrate.run(db.handle) {
      Err(e) => Err(str.concat("migrate failed: ", e)),
      Ok(_) => {
        let cfg := { id: "bs", goal: "g", model: "m", max_iterations: 3, stop_when: "", pmf_when: "", maintenance_when: "", wake_when: "", soft_mesh_url: "", soft_org_id: "", soft_roles: "", soft_settlement: "", policy_isolation: "" }
        let __c := company.save_company(db, cfg)
        let __it := company.record_iteration(db, { company_id: "bs", idx: 1, sprint_id: "bs/iter-1", parent_sprint_id: "", status: "success", goal: "g" })
        let __g1 := tr.save_sprint_graph(db, "bs/iter-1", "Implementation", "{\"id\":\"g\",\"nodes\":[{\"id\":\"build\",\"role\":\"build\",\"gate\":\"spec compiles\"}],\"edges\":[]}")
        let __g2 := tr.save_sprint_graph(db, "bs/iter-1", "Intake", "{\"id\":\"bs-intake\",\"nodes\":[{\"id\":\"intake\",\"role\":\"pm\",\"gate\":\"spec len-gt 50\"}],\"edges\":[]}")
        let __n := tr.write_node_result(db, "bs/iter-1", "build", "Implementation", true, "abc", "")
        let status := company.build_status_section(db, "bs")
        if str.contains(status, "was accepted in the MOST RECENT iteration") {
          Ok(())
        } else {
          Err(str.concat("an accepted Lex build hidden behind a later graph row was reported as never accepted: ", status))
        }
      },
    },
  }
...
fn test_iteration_goal_states_the_empty_work_dir_from_iteration_two() -> Result[Unit, Str] {
  let g2 := company_runner.iteration_goal("Fix the tzconvert test suite so QA passes", 2, "", false, "")
  let g1 := company_runner.iteration_goal("Build tzconvert", 1, "", false, "")
