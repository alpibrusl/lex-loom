...
fn test_strategist_prompt_includes_operate_signals() -> Result[Unit, Str] {
  ...
  let prompt := company_runner.strategist_prompt("Build a widget factory", "widget v1", [], "2026-07-06T12:00:00Z: down", "(no product usage signal recorded yet)", "NO Lex ('build' role) node has EVER been accepted for this company, across every iteration so far.", "Add widget v2", iter_ctx)
...
fn test_strategist_prompt_no_signals_yet() -> Result[Unit, Str] {
  ...
  let prompt := company_runner.strategist_prompt("Build a widget factory", "(empty)", [], "(no launched server for this company, or no liveness checks yet)", "(no product usage signal recorded yet)", "NO Lex ('build' role) node has EVER been accepted for this company, across every iteration so far.", "Ship v1", iter_ctx)
...
fn test_strategist_prompt_includes_product_signals() -> Result[Unit, Str] {
  ...
  let prompt := company_runner.strategist_prompt("Build a feedback tool", "feedback v1", [], "2026-07-06T12:00:00Z: up", "{\"summary\": \"3 projects, 40 feedback rows, avg score 6.2\"}", "NO Lex ('build' role) node has EVER been accepted for this company, across every iteration so far.", "Add sentiment trend", iter_ctx)
        ...
        match seed_metrics_incident(db, rocky, "-a", "escalated", "falsified", 100, "2026-01-01T00:00:00Z") {
          ...
          Ok(_) => match seed_metrics_incident(db, rocky, "-b", "escalated", "falsified", 200, "2026-01-01T00:05:00Z") {
            ...
            Ok(_) => {
              ...
              let clean_prompt := company_runner.strategist_prompt("Build a widget factory", "widget v1", [], clean_operate, "(no product usage signal recorded yet)", "build status n/a", "Add widget v2", iter_ctx)
              let rocky_prompt := company_runner.strategist_prompt("Build a widget factory", "widget v1", [], rocky_operate, "(no product usage signal recorded yet)", "build status n/a", "Add widget v2", iter_ctx)
