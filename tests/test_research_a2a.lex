# test_research_a2a.lex — regression coverage for the research A2A token
# gate (lex-loom#193): src/server/research_a2a.lex.
...
import "../src/server/research_a2a" as research_a2a
...
fn test_token_matches_identical_strings() -> Result[Unit, Str] {
  if research_a2a.token_matches("secret-abc", "secret-abc") {
    Ok(())
  } else {
...
fn test_token_matches_rejects_wrong_token() -> Result[Unit, Str] {
  if research_a2a.token_matches("wrong-token", "secret-abc") {
...
fn test_token_matches_rejects_empty_presented() -> Result[Unit, Str] {
  if research_a2a.token_matches("", "secret-abc") {
...
fn test_token_matches_rejects_empty_expected() -> Result[Unit, Str] {
  if research_a2a.token_matches("anything", "") {
...
fn test_is_authorized_with_correct_bearer_token() -> Result[Unit, Str] {
  ...
  if research_a2a.is_authorized(c, "secret-abc") {
    Ok(())
  } else {
