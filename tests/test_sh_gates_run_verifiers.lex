...
fn test_legitimate_gates_stay_valid() -> Result[Unit, Str] {
  let ok_gates := ["spec sh \"bash $LOOM_ROOT/bin/check-imports.sh .\"", "spec sh \"bash $LOOM_ROOT/bin/check-derived-values.sh .\"", "spec sh \"pytest -q\"", "spec sh \"python3 -m pytest -q\"", "spec sh \"docker build -t tzconvert .\"", "spec sh \"npm ci && npm run build\"", "spec sh \"lex test\""]
