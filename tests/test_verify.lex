...
fn test_runtime_matches_policy() -> [env] Result[Unit, Str] {
  list.fold(["build", "py_build", "qa", "py_qa", "launch", "deploy", "content_creator", "cx", "research", "opportunity_research", "founder"], Ok(()), fn (acc :: Result[Unit, Str], r :: Str) -> [env] Result[Unit, Str] {
    match acc {
      Err(e) => Err(e),
      Ok(_) => {
